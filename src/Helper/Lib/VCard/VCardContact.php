...
namespace Weirdo\Helper\Lib\VCard;
...
class VCardContact
{
    ...
    protected function parseTel(string $strValue, array $aParams) : void
    {
        ...
        if (isset($aParams['TYPE'])) {
            ...
            $this->addPhone($strValue, VCard::CELL, false);
        }
    }
    ...
    public function getPhone($i) : ?array
    {
        ...
        if (is_numeric($i)) {
            ...
            foreach ($this->aPhone as $aPhone) {
                if (isset($aPhone['strType']) && strpos($aPhone['strType'], $i) !== false) {
                    return $aPhone;
                }
                $aPhone = null;
            }
