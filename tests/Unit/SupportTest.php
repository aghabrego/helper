...
class SupportTest extends TestCase
{
     ...
     * @var string
     */
    private $fullUrl = "http://user:pass@www.example.com:8080/pub/index.php?a=b#files";
    ...
    {
        $expected = $this->fullUrl;
        $actual = http_build_url($this->fullUrl, array('query' => array('foo' => 'bar')));
    ...
    public function testAcceptStrings()
    {
        ...
        $actual = http_build_url($this->fullUrl, 'http://foobar.com:8080');
    ...
    {
        $expected = 'http://user:pass@foobar.com:8080/pub/index.php?a=b#files';
        $actual = http_build_url(parse_url($this->fullUrl), parse_url('http://foobar.com:8080'));
    ...
    {
        $expected = $this->fullUrl;
        $actual = http_build_url($this->fullUrl);
    ...
    {
        $expected = parse_url($this->fullUrl);
        http_build_url($this->fullUrl, null, null, $actual);
    ...
    {
        $actual = http_build_url($this->fullUrl, array('query' => $query), HTTP_URL_JOIN_QUERY);
    ...
    {
        $actual = http_build_url($this->fullUrl, array('path' => $path), HTTP_URL_JOIN_PATH);
    ...
    {
        $actual = http_build_url($this->fullUrl, array(), constant($constant));
